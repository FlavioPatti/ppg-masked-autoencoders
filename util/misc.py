...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self):
        """
        Un errore comune nelle reti neurali profonde è il problema dell'underflowing gradients (ovvero che i gradienti sono troppi piccoli
        per essere presi in considerazione). In quanto tensori float16 spesso non prendono in considerazioni piccole variazioni.
        Per prevenire questo solitamente si scalano i gradienti di una quantita, in modo che essi non convergano a 0. 
        GradScaler fa questo
        """
...
def all_reduce_mean(x):
    """Ci sono vari gruppi, ogni gruppo esegue il proprio training attraverso una gpu assegnata
    ed è costituito da vari processi ognuno dei quali lavora con un sample distribuito su un sottoinsieme del training. 
    Il rank di un processo è il suo ID all'interno del gruppo"""
    world_size = get_world_size() #  Returns the number of processes in the current process group
    if world_size > 1:
